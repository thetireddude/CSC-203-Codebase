...
public class Dude_Full implements EntityInterface, ExecuteEntityActivity, TransformEntity, DudeFairyInterface, ScheduleAction{
    ...
    public Dude_Full(String kind, String id, Point position, List<PImage> images, int resourceLimit, double actionPeriod, double animationPeriod, int health) {
        this.kind = kind;
        this.id = id;
    ...
    public static Dude_Full createDudeFull(String id, Point position, double actionPeriod, double animationPeriod, int resourceLimit, List<PImage> images) {
        return new Dude_Full("DUDE_FULL", id, position, images, resourceLimit, actionPeriod, animationPeriod, 0);
    }

    public void executeEntityActivity(WorldModel world, ImageStore imageStore, EventScheduler scheduler) {
        Optional<EntityInterface> fullTarget = world.findNearest(this.position, new ArrayList<>(List.of("HOUSE")));
    ...
    public boolean transformEntity(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        EntityInterface dude = Dude_Not_Full.createDudeNotFull(this.id, this.position, this.actionPeriod, this.animationPeriod, this.resourceLimit, this.images);
