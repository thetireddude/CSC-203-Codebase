    ...
    */
    ...
    private void parseSapling(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.SAPLING_NUM_PROPERTIES) {
            ...
            EntityInterface entity = Sapling.createSapling(id, pt, imageStore.getImageList(Entity.SAPLING_KEY), health);
    ...
    private void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.DUDE_NUM_PROPERTIES) {
            EntityInterface entity = Dude_Not_Full.createDudeNotFull(id, pt, Double.parseDouble(properties[Entity.DUDE_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.DUDE_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Entity.DUDE_RESOURCE_LIMIT_IDX]), imageStore.getImageList(Entity.DUDE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseFairy(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.FAIRY_NUM_PROPERTIES) {
            EntityInterface entity = Fairy.createFairy(id, pt, Double.parseDouble(properties[Entity.FAIRY_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.FAIRY_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.FAIRY_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseTree(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.TREE_NUM_PROPERTIES) {
            EntityInterface entity = Tree.createTree(id, pt, Double.parseDouble(properties[Entity.TREE_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Entity.TREE_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Entity.TREE_HEALTH_IDX]), imageStore.getImageList(Entity.TREE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseObstacle(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.OBSTACLE_NUM_PROPERTIES) {
            EntityInterface entity = Obstacle.createObstacle(id, pt, Double.parseDouble(properties[Entity.OBSTACLE_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Entity.OBSTACLE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseHouse(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.HOUSE_NUM_PROPERTIES) {
            EntityInterface entity = House.createHouse(id, pt, imageStore.getImageList(Entity.HOUSE_KEY));
            this.tryAddEntity(entity);
        }else{
    ...
    private void parseStump(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Entity.STUMP_NUM_PROPERTIES) {
            EntityInterface entity = Stump.createStump(id, pt, imageStore.getImageList(Entity.STUMP_KEY));
            this.tryAddEntity(entity);
        }else{
        ...
        while(saveFile.hasNextLine()){
            ...
            if(line.endsWith(":")){
                ...
                switch (line){
                    ...
                    case "Entities:" -> {
                        this.occupancy = new EntityInterface[this.numRows][this.numCols];
    ...
    public void removeEntityAt(Point pos) {
        if (withinBounds(pos) && this.getOccupancyCell(pos) != null) {
            EntityInterface entity = this.getOccupancyCell(pos);
    ...
    public void removeEntity(EventScheduler scheduler, EntityInterface entity) {
        scheduler.unscheduleAllEvents(entity);
        this.removeEntityAt(entity.getPosition());
    }

    public void moveEntity(EventScheduler scheduler, EntityInterface entity, Point pos) {
        Point oldPos = entity.getPosition();
        if (withinBounds(pos) && !pos.equals(oldPos)) {
            this.setOccupancyCell(oldPos, null);
            Optional<EntityInterface> occupant = this.getOccupant(pos);
    ...
    public Optional<EntityInterface> findNearest(Point pos, List<String> kinds) {
